...
public class BlockNuclearFurnace extends BlockContainer {
	...
	{
		return Item.getItemFromBlock(NCBlocks.nuclearFurnaceIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		 ...
		 {
			 FMLNetworkHandler.openGui(player, NuclearCraft.instance, NuclearCraft.guiIdNuclearFurnace, world, x, y, z);
		 }
		 return true;
	 }
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack itemstack)
	{
		 int l = MathHelper.floor_double((double)(entityLivingBase.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
	        ...
	        {
	            ((TileNuclearFurnace)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
	        }
	    }
	...
	public static void updateNuclearFurnaceBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord)
	{
		...
		if(active)
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.nuclearFurnaceActive);
		}
		else
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.nuclearFurnaceIdle);
		}
	...
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata)
	{
		if(!keepInventory)
		{
			TileNuclearFurnace tileentity = (TileNuclearFurnace) world.getTileEntity(x, y, z);
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player) {
		return new ItemStack(NCBlocks.nuclearFurnaceIdle);
	}
}