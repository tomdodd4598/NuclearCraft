...
public class BlockElectricFurnace extends BlockContainer {
	...
	public BlockElectricFurnace(boolean isActive)
	{
	super(Material.iron);
	...
	{
	this.blockIcon = iconRegister.registerIcon("nc:machine/electricFurnace/" + "side");
	this.iconFront = iconRegister.registerIcon("nc:machine/electricFurnace/" + "front" + (this.isActive ? "Active" : "Idle"));
	this.iconTop = iconRegister.registerIcon("nc:machine/electricFurnace/" + "top");
	this.iconBottom = iconRegister.registerIcon("nc:machine/electricFurnace/" + "bottom");
	}

	public Item getItemDropped(int par1, Random random, int par3)
	{
		return Item.getItemFromBlock(NCBlocks.electricFurnaceIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		 ...
		 {
			 FMLNetworkHandler.openGui(player, NuclearCraft.instance, NuclearCraft.guiIdElectricFurnace, world, x, y, z);
		 }
		 return true;
	 }
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack itemstack)
	{
		 int l = MathHelper.floor_double((double)(entityLivingBase.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
	        ...
	        {
	            ((TileElectricFurnace)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
	        }
	    }
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.electricFurnaceActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.electricFurnaceIdle);
		}
		keepInventory = false;
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player) {
		return new ItemStack(NCBlocks.electricFurnaceIdle);
	}
}