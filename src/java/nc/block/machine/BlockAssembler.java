...
public class BlockAssembler extends BlockMachine {
	public BlockAssembler(boolean active) {
		super(active, NuclearCraft.guiIdAssembler, "reddust", "fireworksSpark", "assembler");
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.assemblerActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.assemblerIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NCBlocks.assemblerIdle);
	}
	
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player) {
		return new ItemStack(NCBlocks.assemblerIdle);
	}
	
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileAssembler tileentity = (TileAssembler) world.getTileEntity(x, y, z);
			if(tileentity != null) {
