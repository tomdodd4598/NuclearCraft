...
public class BlockFurnace extends BlockContainer {
	...
	this.iconTop = iconRegister.registerIcon("nc:machine/furnace/" + "top");
	this.iconBottom = iconRegister.registerIcon("nc:machine/furnace/" + "bottom");
	}

	public Item getItemDropped(int par1, Random random, int par3)
	{
		return Item.getItemFromBlock(NCBlocks.furnaceIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		 ...
		 {
			 FMLNetworkHandler.openGui(player, NuclearCraft.instance, NuclearCraft.guiIdFurnace, world, x, y, z);
		 }
		 return true;
	 }
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack itemstack)
	{
		 int l = MathHelper.floor_double((double)(entityLivingBase.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
	        ...
	        {
	            ((TileFurnace)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
	        }
	    }
	...
	public static void updateFurnaceBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord)
	{
		...
		if(active)
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.furnaceActive);
		}
		else
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.furnaceIdle);
		}
	...
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata)
	{
		if(!keepInventory)
		{
			TileFurnace tileentity = (TileFurnace) world.getTileEntity(x, y, z);
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player) {
		return new ItemStack(NCBlocks.furnaceIdle);
	}
