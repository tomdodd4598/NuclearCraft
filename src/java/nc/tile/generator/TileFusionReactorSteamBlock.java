...
public class TileFusionReactorSteamBlock extends TileEntity implements IFluidHandler, IEnergyHandler, IEnergyConnection, IEnergyReceiver, IEnergyProvider, ISidedInventory, IGasHandler, ITubeConnection {
	...
	public TileFusionReactorSteamBlock() {
		super();
    }
	...
	public void updateEntity() {
    	...
    	if (this.worldObj.getBlock(xCoord, yCoord, zCoord) == NCBlocks.fusionReactorSteamBlock && (ppp(xCoord + 1, yCoord, zCoord) || ppp(xCoord - 1, yCoord, zCoord) || ppp(xCoord, yCoord + 1, zCoord) || ppp(xCoord, yCoord - 1, zCoord) || ppp(xCoord, yCoord, zCoord + 1) || ppp(xCoord, yCoord, zCoord - 1))) {
	...
	{
		if (worldObj.getBlock(x, y-1, z) == NCBlocks.fusionReactorSteam) {xOffset=0;yOffset=-1;zOffset=0;}
		else if (worldObj.getBlock(x+1, y, z) == NCBlocks.fusionReactorSteam) {xOffset=1;yOffset=0;zOffset=0;}
		else if (worldObj.getBlock(x+1, y, z+1) == NCBlocks.fusionReactorSteam) {xOffset=1;yOffset=0;zOffset=1;}
		else if (worldObj.getBlock(x, y, z+1) == NCBlocks.fusionReactorSteam) {xOffset=0;yOffset=0;zOffset=1;}
		else if (worldObj.getBlock(x-1, y, z+1) == NCBlocks.fusionReactorSteam) {xOffset=-1;yOffset=0;zOffset=1;}
		else if (worldObj.getBlock(x-1, y, z) == NCBlocks.fusionReactorSteam) {xOffset=-1;yOffset=0;zOffset=0;}
		else if (worldObj.getBlock(x-1, y, z-1) == NCBlocks.fusionReactorSteam) {xOffset=-1;yOffset=0;zOffset=-1;}
		else if (worldObj.getBlock(x, y, z-1) == NCBlocks.fusionReactorSteam) {xOffset=0;yOffset=0;zOffset=-1;}
		...
		else if (worldObj.getBlock(x+1, y-1, z) == NCBlocks.fusionReactorSteam) {xOffset=1;yOffset=-1;zOffset=0;}
		else if (worldObj.getBlock(x+1, y-1, z+1) == NCBlocks.fusionReactorSteam) {xOffset=1;yOffset=-1;zOffset=1;}
		else if (worldObj.getBlock(x, y-1, z+1) == NCBlocks.fusionReactorSteam) {xOffset=0;yOffset=-1;zOffset=1;}
		else if (worldObj.getBlock(x-1, y-1, z+1) == NCBlocks.fusionReactorSteam) {xOffset=-1;yOffset=-1;zOffset=1;}
		else if (worldObj.getBlock(x-1, y-1, z) == NCBlocks.fusionReactorSteam) {xOffset=-1;yOffset=-1;zOffset=0;}
		else if (worldObj.getBlock(x-1, y-1, z-1) == NCBlocks.fusionReactorSteam) {xOffset=-1;yOffset=-1;zOffset=-1;}
		else if (worldObj.getBlock(x, y-1, z-1) == NCBlocks.fusionReactorSteam) {xOffset=0;yOffset=-1;zOffset=-1;}
	...
	public String getInventoryName() {
		ISidedInventory main = (ISidedInventory)worldObj.getTileEntity(xCoord + xOffset, yCoord + yOffset, zCoord + zOffset);
		if (main == null || isNotReady()) {return "Fusion Reactor Steam";}
	...
	public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
		IFluidHandler main = (IFluidHandler)worldObj.getTileEntity(xCoord + xOffset, yCoord + yOffset, zCoord + zOffset);
		if (main == null || isNotReady()) {return 0;}
		return main.fill(from, resource, doFill);
	}

	public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
		IFluidHandler main = (IFluidHandler)worldObj.getTileEntity(xCoord + xOffset, yCoord + yOffset, zCoord + zOffset);
		if (main == null || isNotReady()) {return null;}
		return main.drain(from, resource, doDrain);
	}

	public FluidStack drain(ForgeDirection from, int maxDrain, boolean doDrain) {
		IFluidHandler main = (IFluidHandler)worldObj.getTileEntity(xCoord + xOffset, yCoord + yOffset, zCoord + zOffset);
		if (main == null || isNotReady()) {return null;}
		return main.drain(from, maxDrain, doDrain);
	}

	public boolean canFill(ForgeDirection from, Fluid fluid) {
		IFluidHandler main = (IFluidHandler)worldObj.getTileEntity(xCoord + xOffset, yCoord + yOffset, zCoord + zOffset);
		if (main == null || isNotReady()) {return false;}
		return main.canFill(from, fluid);
	}

	public boolean canDrain(ForgeDirection from, Fluid fluid) {
		IFluidHandler main = (IFluidHandler)worldObj.getTileEntity(xCoord + xOffset, yCoord + yOffset, zCoord + zOffset);
		if (main == null || isNotReady()) {return false;}
		return main.canDrain(from, fluid);
	}

	public FluidTankInfo[] getTankInfo(ForgeDirection from) {
		IFluidHandler main = (IFluidHandler)worldObj.getTileEntity(xCoord + xOffset, yCoord + yOffset, zCoord + zOffset);
		if (main == null || isNotReady()) {return null;}
		return main.getTankInfo(from);
	}
}