...
public class ItemFuel extends Item {
	...
	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < 6; i ++) {
	        this.icons[i] = reg.registerIcon("nr:fuel/" + (i));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons1[i] = reg.registerIcon("nr:fuel/" + (i+6));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons2[i] = reg.registerIcon("nr:fuel/" + (i+12));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons3[i] = reg.registerIcon("nr:fuel/" + (i+18));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons4[i] = reg.registerIcon("nr:fuel/" + (i+24));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons5[i] = reg.registerIcon("nr:fuel/" + (i+30));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons6[i] = reg.registerIcon("nr:fuel/" + (i+36));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons7[i] = reg.registerIcon("nr:fuel/" + (i+42));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons8[i] = reg.registerIcon("nr:fuel/" + (i+48));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons9[i] = reg.registerIcon("nr:fuel/" + (i+54));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons10[i] = reg.registerIcon("nr:fuel/" + (i+60));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons11[i] = reg.registerIcon("nr:fuel/" + (i+66));
	    }
	    for (int i = 0; i < 4; i ++) {
	        this.icons12[i] = reg.registerIcon("nr:fuel/" + (i+72));
	    }
	}
	    ...
	    if (stack.getItemDamage() == 45) {
		    ...
		    if (movingobjectposition == null) {
		    	...
		    	if (material == Material.water && l == 0) {
		            ...
		            return this.func_150910_a(stack, player, new ItemStack(NRItems.fuel, 1, 34));
		        }
		    	if (material == NuclearRelativistics.liquidhelium && l == 0) {
		    		world.setBlockToAir(i, j, k);
		            return this.func_150910_a(stack, player, new ItemStack(NRItems.fuel, 1, 75));
		        }
	    	}
	    }
	    return stack;
