...
public class ItemMaterial extends Item {
	...
	public IIcon[] icons11 = new IIcon[5];

	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < 6; i ++) {
	        this.icons[i] = reg.registerIcon("nr:materials/" + (i));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons1[i] = reg.registerIcon("nr:materials/" + (i+6));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons2[i] = reg.registerIcon("nr:materials/" + (i+12));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons3[i] = reg.registerIcon("nr:materials/" + (i+18));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons4[i] = reg.registerIcon("nr:materials/" + (i+24));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons5[i] = reg.registerIcon("nr:materials/" + (i+30));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons6[i] = reg.registerIcon("nr:materials/" + (i+36));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons7[i] = reg.registerIcon("nr:materials/" + (i+42));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons8[i] = reg.registerIcon("nr:materials/" + (i+48));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons9[i] = reg.registerIcon("nr:materials/" + (i+54));
	    }
	    for (int i = 0; i < 6; i ++) {
	        this.icons10[i] = reg.registerIcon("nr:materials/" + (i+60));
	    }
	    for (int i = 0; i < 5; i ++) {
	        this.icons11[i] = reg.registerIcon("nr:materials/" + (i+66));
	    }
	}
	...
	public IIcon getIconFromDamage(int meta) {
	    ...
	    else if (meta >= 66 && meta < 71) { return this.icons11[meta-66];}
	...
	public String getUnlocalizedName(ItemStack stack) {
	    ...
	    case 48: return "B10";
	    case 49: return "B11";
	    case 50: return "aluminiumIngot";
	    case 51: return "crushedAluminium";
