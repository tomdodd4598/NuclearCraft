...
public class BlockElectrolyser extends BlockMachine {
	...
	public BlockElectrolyser(boolean active) {
		super(active, NuclearRelativistics.guiIdElectrolyser, "", "");
	}

	@SideOnly(Side.CLIENT)
	@Override
	public void registerBlockIcons(IIconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon("nr:machines/" + "electrolyserSide");
		this.iconFront = iconRegister.registerIcon("nr:machines/" + "electrolyser" + "Front" + (this.isActive ? "Active" : "Idle"));
		this.iconBottom = iconRegister.registerIcon("nr:machines/" + "nuclearMachineSide");
	}
	
	@SideOnly(Side.CLIENT)
	public IIcon getIcon(int side, int metadata) {
		return side == 0 ? this.iconBottom : (side == 1 ? this.blockIcon : this.iconFront);
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.electrolyserActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.electrolyserIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NRBlocks.electrolyserIdle);
	}
	
	public Block idPicked(World world, int x, int y, int z) {
		return NRBlocks.electrolyserIdle;
	}
	
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileEntityElectrolyser tileentity = (TileEntityElectrolyser) world.getTileEntity(x, y, z);
			if(tileentity != null) {
