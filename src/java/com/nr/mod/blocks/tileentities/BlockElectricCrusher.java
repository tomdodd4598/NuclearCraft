...
public class BlockElectricCrusher extends BlockMachine {
	public BlockElectricCrusher(boolean active) {
		super(active, NuclearRelativistics.guiIdElectricCrusher, "smoke", "smoke");
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.electricCrusherActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.electricCrusherIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NRBlocks.electricCrusherIdle);
	}
	
	public Block idPicked(World world, int x, int y, int z) {
		return NRBlocks.electricCrusherIdle;
	}
	
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileEntityElectricCrusher tileentity = (TileEntityElectricCrusher) world.getTileEntity(x, y, z);
			if(tileentity != null) {
