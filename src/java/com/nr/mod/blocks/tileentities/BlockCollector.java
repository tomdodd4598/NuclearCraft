...
public class BlockCollector extends BlockMachine {
	public BlockCollector(boolean active) {
		super(active, NuclearRelativistics.guiIdCollector, "", "depthsuspend");
	}

	@SideOnly(Side.CLIENT)
	@Override
	public void registerBlockIcons(IIconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon("nr:machines/" + "collector");
		this.iconFront = iconRegister.registerIcon("nr:machines/" + "nuclearMachineSide");
	}

	@SideOnly(Side.CLIENT)
	public IIcon getIcon(int side, int metadata) {
		return side == 0 ? this.iconFront : this.blockIcon;
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.collectorActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.collectorIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NRBlocks.collectorIdle);
	}
	
	public Block idPicked(World world, int x, int y, int z) {
		return NRBlocks.collectorIdle;
	}
