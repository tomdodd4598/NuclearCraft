...
public class BlockReactionGenerator extends BlockContainer {
	...
	{
	this.blockIcon = iconRegister.registerIcon("nr:machines/" + "reactionGenerator"+ (this.isActive ? "Active" : "Idle"));
	}

	public Item getItemDropped(int par1, Random random, int par3)
	{
		return Item.getItemFromBlock(NRBlocks.reactionGeneratorIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		 ...
		 {
			 FMLNetworkHandler.openGui(player, NuclearRelativistics.instance, NuclearRelativistics.guiIdReactionGenerator, world, x, y, z);
		 }
		 return true;
	 }
	...
	{
		return new TileEntityReactionGenerator();
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack itemstack)
	{
		 int l = MathHelper.floor_double((double)(entityLivingBase.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
	        ...
	        {
	            ((TileEntityReactionGenerator)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
	        }
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord)
	{
		...
		if(active)
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.reactionGeneratorActive);
		}
		else
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.reactionGeneratorIdle);
		}
	...
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata)
	{
		if(!keepInventory)
		{
			TileEntityReactionGenerator tileentity = (TileEntityReactionGenerator) world.getTileEntity(x, y, z);
	...
	{
		return NRBlocks.reactionGeneratorIdle;
	}
