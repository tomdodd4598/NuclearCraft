...
public class BlockSeparator extends BlockMachine {
	public BlockSeparator(boolean active) {
		super(active, NuclearRelativistics.guiIdSeparator, "reddust", "smoke");
	}

	@SideOnly(Side.CLIENT)
	public void registerBlockIcons(IIconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon("nr:machines/" + "nuclearMachineSide");
		this.iconFront = iconRegister.registerIcon("nr:machines/" + "separator" + "Front" + (this.isActive ? "Active" : "Idle"));
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.separatorActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.separatorIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NRBlocks.separatorIdle);
	}
	
	public Block idPicked(World world, int x, int y, int z) {
		return NRBlocks.separatorIdle;
	}
	
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileEntitySeparator tileentity = (TileEntitySeparator) world.getTileEntity(x, y, z);
			if(tileentity != null) {
