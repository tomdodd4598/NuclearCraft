...
public class BlockCooler extends BlockMachine {
	public BlockCooler(boolean active) {
		super(active, NuclearRelativistics.guiIdCooler, "", "depthsuspend");
	}

	@SideOnly(Side.CLIENT)
	public void registerBlockIcons(IIconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon("nr:machines/" + "nuclearMachineSide");
		this.iconFront = iconRegister.registerIcon("nr:machines/" + "cooler" + "Front" + (this.isActive ? "Active" : "Idle"));
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.coolerActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.coolerIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NRBlocks.coolerIdle);
	}
	
	public Block idPicked(World world, int x, int y, int z) {
		return NRBlocks.coolerIdle;
	}
	
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileEntityCooler tileentity = (TileEntityCooler) world.getTileEntity(x, y, z);
			if(tileentity != null) {
