...
public class BlockElectricFurnace extends BlockContainer {
	...
	this.iconFront = iconRegister.registerIcon("nr:machines/" + "electricFurnace" + "Front" + (this.isActive ? "Active" : "Idle"));
	}

	public Item getItemDropped(int par1, Random random, int par3)
	{
		return Item.getItemFromBlock(NRBlocks.electricFurnaceIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		 ...
		 {
			 FMLNetworkHandler.openGui(player, NuclearRelativistics.instance, NuclearRelativistics.guiIdElectricFurnace, world, x, y, z);
		 }
		 return true;
	 }
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack itemstack)
	{
		 int l = MathHelper.floor_double((double)(entityLivingBase.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
	        ...
	        {
	            ((TileEntityElectricFurnace)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
	        }
	    }
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.electricFurnaceActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.electricFurnaceIdle);
		}
		keepInventory = false;
	...
	{
		return NRBlocks.electricFurnaceIdle;
	}
