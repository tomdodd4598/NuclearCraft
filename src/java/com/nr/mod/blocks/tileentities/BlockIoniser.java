...
public class BlockIoniser extends BlockMachine {
	public BlockIoniser(boolean active) {
		super(active, NuclearRelativistics.guiIdIoniser, "reddust", "reddust");
	}

	@SideOnly(Side.CLIENT)
	public void registerBlockIcons(IIconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon("nr:machines/" + "nuclearMachineSide");
		this.iconFront = iconRegister.registerIcon("nr:machines/" + "ioniser" + "Front" + (this.isActive ? "Active" : "Idle"));
	}
	
	public TileEntity createNewTileEntity(World world, int par1) {
		return new TileEntityIoniser();
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.ioniserActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.ioniserIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NRBlocks.ioniserIdle);
	}
	
	public Block idPicked(World world, int x, int y, int z) {
		return NRBlocks.ioniserIdle;
	}
	
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileEntityIoniser tileentity = (TileEntityIoniser) world.getTileEntity(x, y, z);
			if(tileentity != null) {
