...
public class BlockFactory extends BlockMachine {
	public BlockFactory(boolean active) {
		super(active, NuclearRelativistics.guiIdFactory, "fireworksSpark", "reddust");
	}

	@SideOnly(Side.CLIENT)
	public void registerBlockIcons(IIconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon("nr:machines/" + "nuclearMachineSide");
		this.iconFront = iconRegister.registerIcon("nr:machines/" + "factory" + "Front" + (this.isActive ? "Active" : "Idle"));
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.factoryActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NRBlocks.factoryIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NRBlocks.factoryIdle);
	}
	
	public Block idPicked(World world, int x, int y, int z) {
		return NRBlocks.factoryIdle;
	}
	
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileEntityFactory tileentity = (TileEntityFactory) world.getTileEntity(x, y, z);
			if(tileentity != null) {
