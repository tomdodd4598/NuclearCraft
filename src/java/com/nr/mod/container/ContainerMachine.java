...
import com.nr.mod.blocks.tileentities.TileEntityMachine;
import com.nr.mod.crafting.NRRecipeHelper;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public abstract class ContainerMachine extends Container {
	public TileEntityMachine entity;
	public NRRecipeHelper recipes;
	public int lastCookTime;
	public int lastEnergy;
	public int lastEU;
	public int lastSU;
  
	public ContainerMachine(InventoryPlayer inventory, TileEntityMachine tileentity, NRRecipeHelper recipe) {
		entity = tileentity;
		recipes = recipe;
	}
	...
	public void detectAndSendChanges() {
		...
		for (int i = 0; i < crafters.size(); i++) {
			...
			if (entity.hasUpgrades) {
				icrafting.sendProgressBarUpdate(this, 100, entity.energyUpgrade);
				icrafting.sendProgressBarUpdate(this, 101, entity.energyUpgrade >> 16);
				icrafting.sendProgressBarUpdate(this, 102, entity.speedUpgrade);
				icrafting.sendProgressBarUpdate(this, 103, entity.speedUpgrade >> 16);
			}
		}
	...
	public void updateProgressBar(int slot, int value) {
		...
		if (entity.hasUpgrades) {
			...
			if (slot == 101) {
				entity.energyUpgrade = lastEU | value << 16;
			}
			if (slot == 102) {
				lastSU = upcastShort(value);
			}
			if (slot == 103) {
				entity.speedUpgrade = lastSU | value << 16;
			}
		}
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int clickedSlotNumber) {
		...
		if ((slot != null) && (slot.getHasStack())) {
			...
			if (clickedSlotNumber >= entity.inputSize && clickedSlotNumber < entity.inputSize + entity.outputSize) {
				...
				if (TileEntityMachine.isSpeedUpgrade(itemstack1) && entity.hasUpgrades) {
					if (!mergeItemStack(itemstack1, sSlot, sSlot + 1, false)) {
						return null;
					}
				}
				else if (TileEntityMachine.isEnergyUpgrade(itemstack1) && entity.hasUpgrades) {
