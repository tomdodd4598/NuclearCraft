...
public abstract class ContainerMachineFluidIn extends Container {
	...
	public ContainerMachineFluidIn(InventoryPlayer inventory, TileMachineFluidIn tileentity, NCRecipeHelper recipe) {
		entity = tileentity;
		recipes = recipe;
	}
	...
	public void updateProgressBar(int slot, int value) {
		...
		if (slot == 300) {
			lastFluid = upcastShort(value);
		}
		if (slot == 301) {
			entity.fluid = lastFluid | value << 16;
		}
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int clickedSlotNumber) {
		...
		if ((slot != null) && (slot.getHasStack())) {
			...
			if (clickedSlotNumber >= entity.inputSize && clickedSlotNumber < entity.inputSize + entity.outputSize) {
				...
				if (TileMachineFluidIn.isSpeedUpgrade(itemstack1) && entity.hasUpgrades) {
					if (!mergeItemStack(itemstack1, sSlot, sSlot + 1, false)) {
						return null;
					}
				}
				else if (TileMachineFluidIn.isEnergyUpgrade(itemstack1) && entity.hasUpgrades) {
