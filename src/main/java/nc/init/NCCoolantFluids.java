...
public class NCCoolantFluids {
	...
	private static void addMoltenPair(String name, int color) {
		...
		fluidPairList.add(Pair.of(fluid, register_fluid_blocks || ModCheck.distantHorizonsLoaded() ? new BlockFluidMolten(fluid) : null));
	}
	...
	private static void addNAKPairs(String name, int color) {
		...
		fluidPairList.add(Pair.of(coolant, register_fluid_blocks || ModCheck.distantHorizonsLoaded() ? new BlockFluidCoolant(coolant) : null));
		FluidHotCoolant hotCoolant = new FluidHotCoolant(name + "_nak_hot", FluidHotCoolant.getNAKColor(color));
		fluidPairList.add(Pair.of(hotCoolant, register_fluid_blocks || ModCheck.distantHorizonsLoaded() ? new BlockFluidHotCoolant(hotCoolant) : null));
	}
