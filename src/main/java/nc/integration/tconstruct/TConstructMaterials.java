...
public class TConstructMaterials {
	...
	public static void init() {
		if (NCConfig.tool_tic_register[0] || (ModCheck.constructsArmoryLoaded() && NCConfig.armor_conarm_register[0])) registerMaterial("boron", "Boron", "ingotBoron", "boron", true, new ITrait[] {TinkerTraits.dense, TinkerTraits.stiff}, new ITrait[] {TinkerTraits.stiff}, 0x6C6C6C, 0x7D7D7D, 0x959595, false, 0);
		if (NCConfig.tool_tic_register[1] || (ModCheck.constructsArmoryLoaded() && NCConfig.armor_conarm_register[1])) registerMaterial("tough", "Tough", "ingotTough", "tough", true, new ITrait[] {TinkerTraits.dense, TinkerTraits.momentum}, new ITrait[] {TinkerTraits.dense}, 0x140E1F, 0x150F21, 0x171221, false, 1);
		if (NCConfig.tool_tic_register[2] || (ModCheck.constructsArmoryLoaded() && NCConfig.armor_conarm_register[2])) registerMaterial("hard_carbon", "HardCarbon", "ingotHardCarbon", "hard_carbon", true, new ITrait[] {TinkerTraits.lightweight, TinkerTraits.sharp}, new ITrait[] {TinkerTraits.lightweight}, 0x0C284F, 0x195970, 0x227871, false, 2);
		if (NCConfig.tool_tic_register[3] || (ModCheck.constructsArmoryLoaded() && NCConfig.armor_conarm_register[3])) registerMaterial("boron_nitride", "BoronNitride", "gemBoronNitride", null, false, new ITrait[] {TinkerTraits.jagged, TinkerTraits.splintering}, new ITrait[] {TinkerTraits.splintering}, 0xCE75B269, 0xCE75B269, 0xCE75B269, true, 3);
		
		if (NCConfig.tool_tic_register[4] || (ModCheck.constructsArmoryLoaded() && NCConfig.armor_conarm_register[4])) registerMaterial("thorium", "Thorium", "ingotThorium", "thorium", true, new ITrait[] {NCTraits.WITHERING}, new ITrait[] {TinkerTraits.poisonous, NCTraits.WITHERING}, 0x222222, 0x292929, 0x2E2E2E, false, 225, 9D, 2.5D, 2, 0.75D);
		if (NCConfig.tool_tic_register[5] || (ModCheck.constructsArmoryLoaded() && NCConfig.armor_conarm_register[5])) registerMaterial("uranium", "Uranium", "ingotUranium", "uranium", true, new ITrait[] {TinkerTraits.poisonous}, new ITrait[] {TinkerTraits.poisonous, NCTraits.WITHERING}, 0x344F33, 0x406340, 0x4C6B4C, false, 242, 7.5D, 2D, 2, 0.8D);
		if (NCConfig.tool_tic_register[6] || (ModCheck.constructsArmoryLoaded() && NCConfig.armor_conarm_register[6])) registerMaterial("magnesium", "Magnesium", "ingotMagnesium", "magnesium", true, new ITrait[] {TinkerTraits.holy, TinkerTraits.lightweight}, new ITrait[] {TinkerTraits.lightweight}, 0xB898B5, 0xE3BDDF, 0xF7DFF9, false, 197, 11D, 1.5D, 1, 1D);
		if (NCConfig.tool_tic_register[7] || (ModCheck.constructsArmoryLoaded() && NCConfig.armor_conarm_register[7])) registerMaterial("chocolate", "Chocolate", "ingotChocolate", "milk_chocolate", true, new ITrait[] {NCTraits.MOLDABLE_II, TinkerTraits.tasty}, new ITrait[] {NCTraits.MOLDABLE_I, NCTraits.UPLIFTING}, 0x4D180A, 0x581E0C, 0x62230E, false, 32, 2D, 0D, 0, 0.15D);
	}
