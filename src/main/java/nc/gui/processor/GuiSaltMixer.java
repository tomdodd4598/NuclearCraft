...
public class GuiSaltMixer extends GuiFluidProcessor {
	...
	private GuiSaltMixer(EntityPlayer player, TileFluidProcessor tile, ContainerTile container) {
	...
	protected void actionPerformed(GuiButton guiButton) {
		if (tile.getWorld().isRemote) {
			for (int i = 0; i < 3; i++) if (guiButton.id == i && NCUtil.isModifierKeyDown()) {
	...
	public static class SideConfig extends GuiSaltMixer {
		...
		@Override
		public void renderButtonTooltips(int mouseX, int mouseY) {
			drawTooltip(TextFormatting.DARK_AQUA + Lang.localise("gui.container.input_tank_config"), mouseX, mouseY, 45, 34, 18, 18);
