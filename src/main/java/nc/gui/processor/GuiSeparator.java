...
public class GuiSeparator extends GuiItemProcessor {
	...
	public GuiSeparator(EntityPlayer player, TileItemProcessor tile) {
		this(player, tile, new ContainerSeparator(player, tile));
	}
	
	protected GuiSeparator(EntityPlayer player, TileItemProcessor tile, ContainerTile<TileItemProcessor> container) {
		super("separator", player, tile, container);
		xSize = 176;
		ySize = 166;
	}
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		drawTexturedModalRect(guiLeft + 60, guiTop + 34, 176, 3, getCookProgressScaled(37), 18);
	...
	public static class SideConfig extends GuiSeparator {
		...
		public void renderButtonTooltips(int mouseX, int mouseY) {
			...
			drawTooltip(TextFormatting.GOLD + Lang.localise("gui.nc.container.output_item_config"), mouseX, mouseY, 125, 30, 26, 26);
		...
		@Override
		public void initButtons() {
			buttonList.add(new NCButton.SorptionConfig.ItemInput(0, guiLeft + 41, guiTop + 34));
			buttonList.add(new NCButton.SorptionConfig.ItemOutput(1, guiLeft + 97, guiTop + 30));
			buttonList.add(new NCButton.SorptionConfig.ItemOutput(2, guiLeft + 125, guiTop + 30));
