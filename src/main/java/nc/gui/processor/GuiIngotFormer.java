...
public class GuiIngotFormer extends GuiItemFluidProcessor {
	...
	public GuiIngotFormer(EntityPlayer player, TileItemFluidProcessor tile) {
		this(player, tile, new ContainerIngotFormer(player, tile));
	}
	
	private GuiIngotFormer(EntityPlayer player, TileItemFluidProcessor tile, ContainerTile container) {
		super("ingot_former", player, tile, container);
		xSize = 176;
		ySize = 166;
	}
	...
	public static class SideConfig extends GuiIngotFormer {
		...
		@Override
		public void renderButtonTooltips(int mouseX, int mouseY) {
			drawTooltip(TextFormatting.DARK_AQUA + Lang.localise("gui.container.input_tank_config"), mouseX, mouseY, 55, 34, 18, 18);
			drawTooltip(TextFormatting.GOLD + Lang.localise("gui.container.output_item_config"), mouseX, mouseY, 111, 30, 26, 26);
