...
public class GuiTurbineController extends GuiMultiblockController<Turbine> {
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		String expansion_level = Lang.localise("gui.container.turbine_controller.expansion_level") + " " + (multiblock.idealTotalExpansionLevel <= 0D ? "0%" : (NCMath.decimalPlaces(100D*multiblock.totalExpansionLevel, 1) + "% [" + (DoubleMath.isMathematicalInteger(multiblock.idealTotalExpansionLevel) ? new Integer((int)multiblock.idealTotalExpansionLevel).toString() : NCMath.decimalPlaces(multiblock.idealTotalExpansionLevel, 1)) + " x " + NCMath.decimalPlaces(100D*(multiblock.totalExpansionLevel/multiblock.idealTotalExpansionLevel), 1) + "%]"));
