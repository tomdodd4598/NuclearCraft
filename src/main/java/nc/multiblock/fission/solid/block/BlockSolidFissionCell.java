...
public class BlockSolidFissionCell extends BlockFissionPart implements ISidedProperty<SolidFissionCellSetting> {
	 ...
	 * 
	 * if (ItemMultitool.isMultitool(player.getHeldItem(hand))) { TileEntity tile = world.getTileEntity(pos); if (tile instanceof TileSolidFissionCell) { TileSolidFissionCell vessel = (TileSolidFissionCell) tile; EnumFacing side = player.isSneaking() ? facing.getOpposite() : facing; vessel.toggleCellSetting(side); if (!world.isRemote) player.sendMessage(getToggleMessage(player, vessel, side)); return true; } }
	 * 
	 * return super.onBlockActivated(world, pos, state, player, hand, facing, hitX, hitY, hitZ); }
	 * 
	 * private static TextComponentString getToggleMessage(EntityPlayer player, TileSolidFissionCell vessel, EnumFacing side) { SolidFissionCellSetting setting = vessel.getCellSetting(side); String message = player.isSneaking() ? "nc.block.fluid_toggle_opposite" : "nc.block.fluid_toggle"; TextFormatting color = setting == SolidFissionCellSetting.DEPLETED_OUT ? TextFormatting.LIGHT_PURPLE : (setting == SolidFissionCellSetting.FUEL_SPREAD ? TextFormatting.GREEN : (setting == SolidFissionCellSetting.DEFAULT ? TextFormatting.WHITE : TextFormatting.GRAY)); return new TextComponentString(Lang.localise(message) + " " + color + Lang.localise("nc.block.salt_vessel_fluid_side." + setting.getName())); }
	 * 
	 ...
	 * 
	 * @Override public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) { if (placementSide == null) return; BlockPos from = pos.offset(placementSide); TileEntity tile = world.getTileEntity(pos), otherTile = world.getTileEntity(from); if (tile instanceof TileSolidFissionCell && otherTile instanceof TileSolidFissionCell) { TileSolidFissionCell vessel = (TileSolidFissionCell) tile; TileSolidFissionCell other = (TileSolidFissionCell) otherTile; vessel.setInventoryConnections(InventoryConnection.cloneArray(other. getInventoryConnections())); vessel.setCellSettings(other.getCellSettings().clone()); vessel.markDirtyAndNotify(); } } */
}
