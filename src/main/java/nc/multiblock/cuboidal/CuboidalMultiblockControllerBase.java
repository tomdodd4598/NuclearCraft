...
public abstract class CuboidalMultiblockControllerBase extends MultiblockControllerBase {
	...
	protected CuboidalMultiblockControllerBase(World world) {
		super(world);
	}
	...
	protected boolean isMachineWhole(IMultiblockValidator validatorCallback) {
		...
		TileEntity te;
		CuboidalMultiblockTileEntityBase part;
		Class<? extends CuboidalMultiblockControllerBase> myClass = this.getClass();
		int extremes;
		boolean isPartValid;
		...
		for(int x = minX; x <= maxX; x++) {
			for(int y = minY; y <= maxY; y++) {
				for(int z = minZ; z <= maxZ; z++) {
					...
					if(te instanceof CuboidalMultiblockTileEntityBase) {
						part = (CuboidalMultiblockTileEntityBase)te;
