...
public class FurnaceRecipeHandler {
	...
	public static void registerFurnaceRecipes() {
		...
		for (int i = 0; i < IngotOxideType.values().length; i++) {
			GameRegistry.addSmelting(new ItemStack(NCItems.dust_oxide, 1, i), OreDictHelper.getPrioritisedCraftingStack(new ItemStack(NCItems.ingot_oxide, 1, i), "ingot" + StringHelper.capitalize(IngotOxideType.values()[i].getName()) + "Oxide"), 0F);
		}
		/*MnO2 -> MnO*/ GameRegistry.addSmelting(new ItemStack(NCItems.ingot_oxide, 1, 3), OreDictHelper.getPrioritisedCraftingStack(new ItemStack(NCItems.ingot_oxide, 1, 2), "ingotManganeseOxide"), 0F);
		for (int i = 0; i < IngotOxideType.values().length; i++) {
			GameRegistry.addSmelting(new ItemStack(NCItems.ingot_oxide, 1, i), OreDictHelper.getPrioritisedCraftingStack(new ItemStack(NCItems.ingot_oxide, 1, i), "ingot" + StringHelper.capitalize(IngotOxideType.values()[i].getName())), 0F);
		}
