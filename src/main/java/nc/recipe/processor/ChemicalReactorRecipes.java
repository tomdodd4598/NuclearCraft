...
public class ChemicalReactorRecipes extends ProcessorRecipeHandler {
	...
	public void addElementFluorideRecipes(String... elements) {
		for (String element : elements) addRecipe(fluidStack(element, FluidStackHelper.INGOT_VOLUME), fluidStack("fluorine", FluidStackHelper.BUCKET_VOLUME), fluidStack(element + "_fluoride", FluidStackHelper.INGOT_VOLUME), emptyFluidStack(), 1D, 0.5D);
	}
	
	public void addIsotopeFluorideRecipes(String element, int... types) {
		for (int type : types) addRecipe(fluidStack(element + "_" + type, FluidStackHelper.INGOT_VOLUME), fluidStack("fluorine", FluidStackHelper.BUCKET_VOLUME), fluidStack(element + "_" + type + "_fluoride", FluidStackHelper.INGOT_VOLUME), emptyFluidStack(), 1D, 0.5D);
	}
	...
	public void addFissionFuelFluorideRecipes(String suffix, int... types) {
		for (int type : types) for (String form : new String[] {"fuel_l", "fuel_h", "depleted_fuel_l", "depleted_fuel_h"}) {
			addRecipe(fluidStack(form + suffix + "_" + type, FluidStackHelper.INGOT_VOLUME), fluidStack("fluorine", FluidStackHelper.BUCKET_VOLUME), fluidStack(form + suffix + "_" + type + "_fluoride", FluidStackHelper.INGOT_VOLUME), emptyFluidStack(), 1D, 0.5D);
		}
	}
}
