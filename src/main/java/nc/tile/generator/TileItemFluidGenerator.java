...
public abstract class TileItemFluidGenerator extends TileEnergyFluidSidedInventory implements IInterfaceable, IGui {
	...
	public void consume() {
		...
		if (!hasConsumed) {
			...
			for (int i = 0; i < itemInputSize; i++) {
				if (recipes != null) {
					inventoryStacks[i + itemInputSize + itemOutputSize + otherSlotsSize] = new ItemStack(inventoryStacks[i].getItem(), recipe.inputs().get(itemInputOrder[i]).getStackSize(), inventoryStacks[i].getMetadata());
