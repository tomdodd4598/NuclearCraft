...
public class TileFissionPort extends TileEnergySidedInventory {
	...
	public void onAdded() {
		super.onAdded();
		if (!world.isRemote) findController();
	}
	...
	public int getSizeInventory() {
		return getController() != null ? getController().inventoryStacks.size() : inventoryStacks.size();
	}
	...
	public ItemStack getStackInSlot(int slot) {
		return getController() != null ? getController().inventoryStacks.get(slot) : inventoryStacks.get(slot);
	}
	...
	public void clear() {
		(getController() != null ? getController().inventoryStacks : inventoryStacks).clear();
	}
				...
				for (int x = x0; x <= x1; x++) {
					for (int y = y0; y <= y1; y++) {
						if(world.getTileEntity(position(x, y, z)) != null) {
							if(world.getTileEntity(position(x, y, z)) instanceof TileFissionController) {
	...
	public TileFissionController getController() {
		...
		if (world.getTileEntity(controllerPosition) instanceof TileFissionController) return (TileFissionController) world.getTileEntity(controllerPosition);
		return null;
	}
}
