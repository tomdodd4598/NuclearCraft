...
import nc.tile.fission.TilePebbleFissionController;
...
public class BlockPebbleFissionController extends BlockFissionPart implements IActivatable {
	
	public BlockPebbleFissionController() {
		super();
	...
	public TileEntity createNewTileEntity(World world, int metadata) {
		return new TilePebbleFissionController();
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!world.isRemote) {
			...
			if (tile instanceof TilePebbleFissionController controller) {
				if (controller.isMultiblockAssembled()) {
					NCCriterions.PEBBLE_FISSION_ASSEMBLED.trigger((EntityPlayerMP) player);
