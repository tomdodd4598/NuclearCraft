	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (player != null) {
			...
			if (tileentity instanceof IBattery) {
				if ((player.getHeldItemMainhand() == null || !player.isSneaking()) && world.isRemote) player.addChatMessage(new TextComponentString("Energy Stored: " + ((IBattery) tileentity).getBatteryStorage().getEnergyStored() + " / " + ((IBattery) tileentity).getBatteryStorage().getMaxEnergyStored() + " RF"));
			}
		}
		return true;
	}
