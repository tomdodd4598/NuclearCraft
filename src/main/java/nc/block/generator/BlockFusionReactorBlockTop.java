...
public class BlockFusionReactorBlockTop extends BlockFusionReactor {
	
	public int below = 1;

	public BlockFusionReactorBlockTop() {
		super(Material.iron);
		this.setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F);
			...
			public TileEntity createNewTileEntity(World w, int i) {
				return new TileFusionReactorBlockTop();
			}
			
			public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
				if (!player.isSneaking()) {
					if ((player != null) && (!world.isRemote)) {
						if (world.getBlock(x, y-2, z) == NCBlocks.fusionReactor) FMLNetworkHandler.openGui(player, NuclearCraft.instance, 11, world, x, y-2, z);
						else if (world.getBlock(x+1, y-2, z) == NCBlocks.fusionReactor) FMLNetworkHandler.openGui(player, NuclearCraft.instance, 11, world, x+1, y-2, z);
						else if (world.getBlock(x+1, y-2, z+1) == NCBlocks.fusionReactor) FMLNetworkHandler.openGui(player, NuclearCraft.instance, 11, world, x+1, y-2, z+1);
						else if (world.getBlock(x, y-2, z+1) == NCBlocks.fusionReactor) FMLNetworkHandler.openGui(player, NuclearCraft.instance, 11, world, x, y-2, z+1);
						else if (world.getBlock(x-1, y-2, z+1) == NCBlocks.fusionReactor) FMLNetworkHandler.openGui(player, NuclearCraft.instance, 11, world, x-1, y-2, z+1);
						else if (world.getBlock(x-1, y-2, z) == NCBlocks.fusionReactor) FMLNetworkHandler.openGui(player, NuclearCraft.instance, 11, world, x-1, y-2, z);
						else if (world.getBlock(x-1, y-2, z-1) == NCBlocks.fusionReactor) FMLNetworkHandler.openGui(player, NuclearCraft.instance, 11, world, x-1, y-2, z-1);
						else if (world.getBlock(x, y-2, z-1) == NCBlocks.fusionReactor) FMLNetworkHandler.openGui(player, NuclearCraft.instance, 11, world, x, y-2, z-1);
		...
		public void breakBlock(World world, int x, int y, int z, Block oldblock, int oldMetadata) {
			...
			else if (world.getBlock(x+1, y-2, z) == NCBlocks.fusionReactor) {
			...
			else if (world.getBlock(x+1, y-2, z-1) == NCBlocks.fusionReactor) {
			...
			else if (world.getBlock(x, y-2, z-1) == NCBlocks.fusionReactor) {
