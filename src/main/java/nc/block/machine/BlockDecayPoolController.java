...
public class BlockDecayPoolController extends BlockMachinePart implements IActivatable {
	
	public BlockDecayPoolController() {
		super();
	...
	public TileEntity createNewTileEntity(World world, int metadata) {
		return new TileDecayPoolController();
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!world.isRemote) {
			...
			if (tile instanceof TileDecayPoolController controller) {
				if (controller.isMultiblockAssembled()) {
					NCCriterions.DECAY_POOL_ASSEMBLED.trigger((EntityPlayerMP) player);
