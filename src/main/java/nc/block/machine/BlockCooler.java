...
public class BlockCooler extends BlockMachine {
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.coolerActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.coolerIdle);
		}
		keepInventory = false;
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(NCBlocks.coolerIdle);
	}
	
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player) {
		return new ItemStack(NCBlocks.coolerIdle);
	}
	
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileCooler tileentity = (TileCooler) world.getTileEntity(x, y, z);
			if(tileentity != null) {
