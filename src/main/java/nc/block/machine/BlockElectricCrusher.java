...
public class BlockElectricCrusher extends BlockMachine {
	public BlockElectricCrusher(boolean active) {
		super(active, NuclearCraft.guiIdElectricCrusher, "smoke", "smoke", "electricCrusher");
	}
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		keepInventory = true;
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.electricCrusherActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, NCBlocks.electricCrusherIdle);
		}
		keepInventory = false;
	...
	public void breakBlock(World world, int x, int y, int z, Block oldBlockID, int oldMetadata) {
		if(!keepInventory) {
			TileElectricCrusher tileentity = (TileElectricCrusher) world.getTileEntity(x, y, z);
			if(tileentity != null) {
