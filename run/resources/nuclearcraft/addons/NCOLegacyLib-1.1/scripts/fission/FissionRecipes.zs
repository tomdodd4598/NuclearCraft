...
function addPebbleFissionRecipes(info as FissionFuelInfo) {
	...
	PebbleFission.addRecipe(fuel, depleted, (FissionHelper.TRISO_TIME_MULT * info.time) as int, (FissionHelper.TRISO_HEAT_MULT * info.heat) as int, info.efficiency, (FissionHelper.TRISO_CRIT_MULT * info.crit) as int, (FissionHelper.TRISO_INTRINSIC_FLUX_MULT * info.intrinsicFlux) as int, info.decay, info.prime, (info.fuelRad + info.depletedRad) / 64.0);
}
...
function addSolidFissionRecipes(info as FissionFuelInfo) {
	for i in 0 to 3 {
		...
		SolidFission.addRecipe(fuel, depleted, (FissionHelper.SFR_TIME_MULT[i] * info.time) as int, (FissionHelper.SFR_HEAT_MULT[i] * info.heat) as int, info.efficiency, (FissionHelper.SFR_CRIT_MULT[i] * info.crit) as int, (FissionHelper.SFR_INTRINSIC_FLUX_MULT[i] * info.intrinsicFlux) as int, info.decay, info.prime, (info.fuelRad + info.depletedRad) / 64.0);
	}
}
...
function addSaltFissionRecipes(info as FissionFuelInfo) {
	...
	SaltFission.addRecipe(fuel, depleted, FissionHelper.MSR_TIME_MULT * info.time / 144.0, (FissionHelper.MSR_HEAT_MULT * info.heat) as int, info.efficiency, (FissionHelper.MSR_CRIT_MULT * info.crit) as int, (FissionHelper.MSR_INTRINSIC_FLUX_MULT * info.intrinsicFlux) as int, info.decay, info.prime, (info.fuelRad + info.depletedRad) / 64.0);
}
